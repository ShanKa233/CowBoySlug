...
namespace CowBoySlug.CowBoy.Ability.RopeUse
{
    public class RopeMaster
    {
        ...
        public static void CallBackSpear(Player player)
        {
            ...
            umbilical.used = true;
            if (WhenSpearOnSomeThing(spear, player, range, umbilical)) return;
            ...
            else if (player.input[1].pckp && !player.input[0].pckp && range > 35)
            {
                ...
                spear.throwDir = new IntVector2(Convert.ToInt32(spearToEndPointDir.x), Convert.ToInt32(spearToEndPointDir.y)); ;
