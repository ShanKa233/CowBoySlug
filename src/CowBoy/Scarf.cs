...
namespace CowBoySlug.CowBoy
{
    public static class Scarf
    {
        ...
        public static ConditionalWeakTable<Player, ScarfModule> modules = new ConditionalWeakTable<Player, ScarfModule>();//字典
        ...
        private static void Ribbon_ctor(On.PlayerGraphics.orig_ctor orig, PlayerGraphics self, PhysicalObject ow)
        {
            orig.Invoke(self, ow);
            //加到字典里
            if (HaveScarf.TryGet(self.player, out var value) && value) modules.Add(self.player, new ScarfModule(self.player));
            ...
            {
                module.ribbon[i] = new GenericBodyPart(self, 1, 0.8f, 0.3f, self.player.mainBodyChunk);
            }
        ...
        private static void Ribbon_Update(On.PlayerGraphics.orig_Update orig, PlayerGraphics self)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                ribbon.ConnectToPoint(ribbonPoint, 18, false, 0, self.player.mainBodyChunk.vel, 0.1f, 0);
                Vector2 visualRibbon = player.mainBodyChunk.pos + x * (ChangeRotation(player) == 0 ? 35 : (ChangeRotation(player) * 30));
                ribbon.vel += vel * Custom.LerpMap(Vector2.Distance(ribbon.pos, visualRibbon), 10f, 150f, 0f, 14f, 0.7f);
            }
        }
        ...
        private static void PlayerGraphics_AddToContainer(On.PlayerGraphics.orig_AddToContainer orig, PlayerGraphics self, RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, FContainer newContatiner)
        {
            ...
            {
                for (int j = 0; j < 3; j++) sLeaser.sprites[scarfIndex + j].MoveInFrontOfOtherNode(sLeaser.sprites[i]);
            }
        }
        private static void PlayerGraphics_DrawSprites(On.PlayerGraphics.orig_DrawSprites orig, PlayerGraphics self, RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, UnityEngine.Vector2 camPos)
        {
          orig.Invoke(self,sLeaser,rCam,timeStacker,camPos);
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                Vector2 vector = Vector2.Lerp(self.player.mainBodyChunk.lastPos, self.player.mainBodyChunk.pos, timeStacker)-dir;
                vector = sLeaser.sprites[3].GetPosition() - dir * 6 - per * 2 * ChangeRotation(self.player) + camPos;
                Vector2 ribbonpos = Vector2.Lerp(module.ribbon[i].lastPos, module.ribbon[i].pos, timeStacker);
                ...
                scar.MoveVertice(3, Vector2.Lerp(module.ribbon[i].lastPos, module.ribbon[i].pos, timeStacker) - camPos);
            ...
            TriangleMesh.Triangle[] tris = new TriangleMesh.Triangle[]
       {
            ...
            //添加2个飘带的材质
            for (int i = 0; i < 2; i++) sLeaser.sprites[index + i+1] = new TriangleMesh("Futile_White", tris, false, false);
    ...
    public class ScarfModule
    {
        ...
        {
            for (int i = 0; i < ribbon.Length; i++) ribbon[i].Reset(player.mainBodyChunk.pos);
        }
